...
public final class TelegramSummary {
    ...
    private static void send(StringBuilder sb, final Callback cb) {
        SimpleDateFormat hf = new SimpleDateFormat("HH:mm", Locale.FRANCE);
        sb.append("<i>🐻 CoupleFinance · ").append(hf.format(new Date())).append("</i>");
    ...
    private static void appendMonth(Context ctx, StringBuilder sb,
        ...
        String mood = balance >= 0 ? " 🟢" : " 🔴";
        sb.append("• Solde du mois : <b>").append(sign).append(money(balance)).append("</b>").append(mood).append("\n");
        if (income > 0) {
            int rate = (int) Math.round(balance / income * 100);
            sb.append("• Taux d'épargne : <b>").append(rate).append("%</b>\n");
        }
        sb.append(sep());
    }
    ...
    private static void appendProjection(Context ctx, StringBuilder sb,
        ...
        if (!Double.isNaN(joint) && remainingCharges > 0) {
            ...
            sb.append(projected < 0 ? " ⚠️" : " ✅");
            sb.append("\n");
            if (projected < 0) {
                sb.append("• <i>Pensez à approvisionner le compte joint.</i>\n");
            }
        }
        sb.append(sep());
    }
